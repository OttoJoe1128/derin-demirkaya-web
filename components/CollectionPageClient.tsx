...
export default function CollectionPageClient({ lang = 'tr', dict }: Props) {
  const isEn = lang === 'en';
    ...
    <div className="min-h-screen bg-neutral-50 pt-4 pb-12 sm:pt-6 sm:pb-20 md:pb-24">
      <div className="max-w-7xl mx-auto px-3.5 sm:px-6">
        ...
        <div className="mb-8 sm:mb-14 md:mb-18">
          ...
          >
            {c?.backHome || (isEn ? '← Home' : '← Ana Sayfa')}
          </Link>
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 sm:gap-8 border-b border-neutral-200/80 pb-6 sm:pb-10">
            <div>
              <span className="text-[10px] sm:text-xs font-sans text-neutral-400 uppercase tracking-[0.25em] block mb-1.5 sm:mb-2">
                {c?.brandSub || 'Derin Buse Demirkaya / nonvalue jewel'}
              </span>
              <h1 className="font-serif text-3xl sm:text-6xl md:text-7xl lg:text-8xl text-neutral-950 uppercase tracking-tighter leading-tight">
                {c?.title || (isEn ? 'Archive & Collection' : 'Arşiv & Koleksiyon')}
              </h1>
            ...
            <div className="grid grid-cols-3 sm:flex sm:flex-row gap-2 sm:gap-6 text-neutral-500 text-[10px] sm:text-xs font-mono uppercase tracking-wider bg-white p-3 sm:p-4 border border-neutral-200/60 shadow-xs">
              <div>
                <span className="text-neutral-400 block text-[9px] sm:text-[10px]">
                  {c?.stats?.archive || (isEn ? 'Archive' : 'Arşiv')}
                </span>
                <span className="text-neutral-950 font-bold">
                  {ARTWORKS_DATA.length} {c?.stats?.pieces || (isEn ? 'Pcs' : 'Eser')}
                </span>
              </div>
              <div className="border-l border-neutral-200 pl-2 sm:pl-6">
                <span className="text-neutral-400 block text-[9px] sm:text-[10px]">
                  {c?.stats?.period || (isEn ? 'Period' : 'Dönem')}
                </span>
                <span className="text-neutral-950 font-bold">{c?.stats?.periodVal || '2018–25'}</span>
              </div>
              <div className="border-l border-neutral-200 pl-2 sm:pl-6">
                <span className="text-neutral-400 block text-[9px] sm:text-[10px]">
                  {c?.stats?.axes || (isEn ? 'Axes' : 'Akslar')}
                </span>
                <span className="text-neutral-950 font-bold truncate block">
                  {c?.stats?.axesVal || 'Object/Space'}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="mt-16 sm:mt-24 md:mt-32 pt-10 sm:pt-16 border-t border-neutral-200/80 grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          <div>
            <span className="text-xs font-sans uppercase tracking-[0.2em] text-neutral-400 block mb-2">
              {c?.footer?.craft || (isEn ? 'Craft & Atelier' : 'Zanaat & Atölye')}
            </span>
          ...
          <div>
            <span className="text-xs font-sans uppercase tracking-[0.2em] text-neutral-400 block mb-2">
              {c?.footer?.cert || (isEn ? 'Condition & Certificate' : 'Kondisyon & Sertifika')}
            </span>
            <p className="text-xs font-sans text-neutral-600 leading-relaxed">
              {c?.footer?.certDesc ||
                (isEn
          ...
          <div>
            <span className="text-xs font-sans uppercase tracking-[0.2em] text-neutral-400 block mb-2">
              {c?.footer?.contact || (isEn ? 'Contact & Bespoke Commissions' : 'İletişim & Özel Eser')}
            </span>
            <p className="text-xs font-sans text-neutral-600 leading-relaxed">
              {c?.footer?.contactDesc ||
                (isEn
